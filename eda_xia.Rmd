...
wages$diff_log_clarissa = log(wages$differ7874)
hist(wages$differ7874,xlab="Difference in wage between 1974-1978 (in dollars)",main="Difference in wage between 1974-1978 (in dollars)",col=rainbow(10))
#not normal, take log
wages$logre78 <- log(wages$r78)
hist(wages$differ7874,xlab="Difference in wage between 1974-1978 (in dollars)",main="Distribution of log wage in 1978",col=rainbow(10))

max(wages$differ7874)
min(wages$differ7874)
mean(wages$differ7874)
# Creating a histogram table (just want to look at the frequencies)
br = seq(min(wages$differ7874), max(wages$differ7874),  by=10)
ranges = paste(head(freq$breaks,-1), freq$breaks[-1], sep=" - ")
freq   = hist(wages$differ7874, breaks = br, include.lowest=TRUE, plot=FALSE)
table(ranges, freq$counts)
table(freq$counts)
```
...
ggplot(wages, aes(x=treat_fac,y=differ7874, fill=re78))+
  ...
  labs(title="Wage Difference 78-74 vs Training",
       x="Training (1=trained, 0=untrained)", y="Wage Difference 78-74 in $")+
...
ggplot(wages,aes(x=age_c,y=differ7874))+ geom_point()+geom_smooth(method="lm",col="red3")+labs(title="Wage Difference78-74 VS Age centered",y="Wage78-74 in $",x="Age in years, centered")

ggplot(wages,aes(x=age_c,y=differ7874))+ geom_point()+geom_smooth(method="lm",col="red3")+labs(title="Wage Difference78-74 VS Age centered by Treatment",y="Wage78-74 in $",x="Age in years, centered") +facet_wrap(~treat_fac)

# numerical variable: education year
ggplot(wages,aes(x=educ,y=differ7874))+ geom_point()+geom_smooth(method="lm",col="red3")+labs(title="Wage Difference78-74 VS Years of Education",y="Wage78-74 in $",x="Years of Education")

ggplot(wages,aes(x=educ,y=differ7874))+ geom_point()+geom_smooth(method="lm",col="red3")+labs(title="Wage Difference78-74 VS Yrs of Education by Treatment",y="Wage78-74 in $",x="Years of Education") +facet_wrap(~treat_fac)
...
n <- nrow(model.matrix(model1)); p <- ncol(model.matrix(model1)); 2*p/n
lev_scores <- hatvalues(model1) #can also use influence(regwagecsquares)$hat
plot(lev_scores,col=ifelse(lev_scores > (2*p/n), 'red2', 'navy'),type="h",
     ylab="Leverage score",xlab="Index",main="Leverage Scores for all observations (Final Model)")
...
geom_point(alpha = .7) + geom_hline(yintercept=0,col="red3") + theme_classic() +
labs(title="Final Model Residuals vs Age (centered)",x="Age in yrs.",y="Residuals")

ggplot(wages,aes(x=educ, y=model1$residual)) +
geom_point(alpha = .7) + geom_hline(yintercept=0,col="red3") + theme_classic() +
labs(title="Final Model Residuals vs Education",x="Educ in yrs",y="Residuals")
...
stepwise2$call
glmmodel1 <- glm(formula = nonZero78_fac ~ treat_fac + age_c + re74_c + black_fac + 
    treat_fac:age_c + treat_fac:black_fac + treat_fac:re74_c, 
...
=======
<<<<<<< HEAD
>>>>>>> 3a3ba6b1422e0c6c14212e4a8d6b3edebd387708
---
title: "EDA_xia"
author: "ShufanXia"
date: "9/23/2021"
...
stargazer(model1,report="v*c*s*p")
```
```{r}
model1 <- lm(formula = differ7874 ~ treat_fac + age_c + married_fac + treat_fac:age_c + 
    age_c:married_fac, data = wages)
...
#unmarried, no training
newdata$married_fac<- factor("Unmarried",levels=levels(wages$married_fac))
newdata$treat_fac <- factor("Not-treated",levels= levels(wages$treat_fac))
...
#unmarried, training
newdata$married_fac<- factor("Unmarried",levels=levels(wages$married_fac))
newdata$treat_fac <- factor("Treated",levels= levels(wages$treat_fac))
...
colors <- c("Unmarried and No Training" = "red", "Married and No Training" = "blue", "Unmarried and Training" = "purple","Married and Training"="orange")

p<-ggplot(newdata, aes(x = age_c)) +
  geom_line(aes(y = predprobbaseline, color = "Unmarried and No Training"),size = 1) +
  geom_line(aes(y=predprobMarried,color="Married and No Training"),size=1)+
  geom_line(aes(y=predprobTraining,color="Unmarried and Training"),size=1)+
  geom_line(aes(y=predprobMandT,color="Married and Training"),size=1)+
...
```
```{r}
binnedplot(y=wages$nonZero78,x=wages$educ,xlab="education years",ylab="nonZero",ylim=c(0,1),main = "Binned education year vs Nonzero78")

binnedplot(y=wages$nonZero78,x=wages$re74,xlab="education years",ylab="nonZero",ylim=c(0,1),main = "Binned education year vs Nonzero78")
...
glmmodel1 <- glm(formula = nonZero78_fac ~ treat_fac + age_c + re74_c + black_fac,
    family = binomial, data = wages)
glmmodelx <- glm(formula = nonZero78_fac ~ treat_fac + age_c + re74_c + black_fac + 
    treat_fac:age_c, 
    family = binomial, data = wages)
glmmodely <- glm(formula = nonZero78_fac ~ treat_fac + age_c + re74_c + black_fac + 
    treat_fac:age_c+treat_fac:black_fac, 
    family = binomial, data = wages)
glmmodelz <- glm(formula = nonZero78_fac ~ treat_fac + age_c + re74_c + black_fac + 
    treat_fac:age_c+treat_fac:black_fac+treat_fac:re74_c, 
    family = binomial, data = wages)

anova(glmmodelx,glmmodel1,test="Chisq")
anova(glmmodelx,glmmodely,test="Chisq")
anova(glmmodely,glmmodelz,test="Chisq")
...
```{r}
# polynomial on age
wages$age_c2 <- I(wages$age_c^2)
wages$age_c3 <- I(wages$age_c^3)
glmmodel2<- glm(formula = nonZero78_fac ~ treat_fac + age_c +age_c2+age_c3+ re74_c + black_fac +  treat_fac:age_c + treat_fac:black_fac + treat_fac:re74_c, 
    family = binomial, data = wages)
summary(glmmodel2)
...
```
```{r}
exp(glmmodel2$coefficient)
```
```{r}
# par(mfcol=c(2,1),mai = c(0.8, 0.9, 0.3,0.3))
binnedplot(fitted(glmmodel2),residuals(glmmodel2,"resp"),xlab="Pred. probabilities",ylab="Avg. residuals",main="Binned residual plot",col.pts="navy")
binnedplot(x=wages$age_c,residuals(glmmodel2,"resp"),xlab="age_c",ylab="Avg. residuals",main="Binned residual plot",col.pts="navy")

binnedplot(x=wages$educ,residuals(glmmodel2,"resp"),xlab="educ",ylab="Avg. residuals",main="Binned residual plot",col.pts="navy")

binnedplot(x=wages$re74_c,residuals(glmmodel2,"resp"),xlab="re74_c",ylab="Avg. residuals",main="Binned residual plot",col.pts="navy")
...
wages_newdata$treat_fac <- factor("Not-treated",levels= levels(wages$treat_fac))
predprobbaseline <- predict(glmmodel2,wages_newdata,type='response')

# black, not trained
wages_newdata$black_fac<- factor("Black",levels=levels(wages$black_fac))
wages_newdata$treat_fac <- factor("Not-treated",levels= levels(wages$treat_fac))
predprobBlack <- predict(glmmodel2,wages_newdata,type='response')

# not black, trained
wages_newdata$black_fac<- factor("Not Black",levels=levels(wages$black_fac))
wages_newdata$treat_fac <- factor("Treated",levels= levels(wages$treat_fac))
predprobtrained <- predict(glmmodel2,wages_newdata,type='response')

# black trained
wages_newdata$black_fac<- factor("Black",levels=levels(wages$black_fac))
wages_newdata$treat_fac <- factor("Treated",levels= levels(wages$treat_fac))
predprobBlackTrained <- predict(glmmodel2,wages_newdata,type='response')


### Plot lines by site
colors <- c("Not Black and No Training" = "red", "Black and No Training" = "blue", "Not Black and Training" = "purple","Black and Training"="orange")

p2<-ggplot(wages_newdata, aes(x = age_c)) +
  geom_line(aes(y = predprobbaseline, color = "Not Black and No Training"),size = 1) +
  geom_line(aes(y=predprobBlack,color="Black and No Training"),size=1)+
  geom_line(aes(y=predprobtrained,color="Not Black and Training"),size=1)+
  geom_line(aes(y=predprobBlackTrained,color="Black and Training"),size=1)+
...
```
>>>>>>> 803f930b109531168a179aad09208d62732c2b6c
