...
ggplot(wages, aes(x=train_fac,y=differ7874,fill=train_fac))+
  geom_boxplot()+
  labs(title="Change in earnings between 1978 and 1974 vs trained",
       x="trained or not", y="Diff earnings 78-74")+geom_text(data=p_meds,aes(x=train_fac,y=med,label=med),size=3,vjust=-1.3)
...
newdata <- data.frame(age_c=round(seq(min(wages$age_c),max(wages$age_c),0.1),2))
#unmarried, not trained
newdata$married_fac<- factor("Not Married",levels=levels(wages$married_fac))
newdata$train_fac <- factor("Not trained",levels= levels(wages$train_fac))
predprobbaseline <- predict(model1,newdata)

#married, not trained
newdata$married_fac<- factor("Married",levels=levels(wages$married_fac))
newdata$train_fac <- factor("Not trained",levels= levels(wages$train_fac))
predprobMarried<- predict(model1,newdata)

#unmarried, training
newdata$married_fac<- factor("Not Married",levels=levels(wages$married_fac))
newdata$train_fac <- factor("Trained",levels= levels(wages$train_fac))
predprobTraining<- predict(model1,newdata)

#married, training
newdata$married_fac<- factor("Married",levels=levels(wages$married_fac))
newdata$train_fac <- factor("Trained",levels= levels(wages$train_fac))
predprobMandT <- predict(model1,newdata)

colors <- c("Not married and Not trained" = "red", "Married and Not traineed" = "blue", "Not Married and Trained" = "purple","Married and Trained"="orange")
p<-ggplot(newdata, aes(x = age_c)) +
  geom_line(aes(y = predprobbaseline, color = "Not married and Not trained"),size = 1) +
  geom_line(aes(y=predprobMarried,color="Married and Not trained"),size=1)+
  geom_line(aes(y=predprobTraining,color="Not married and Trained"),size=1)+
  geom_line(aes(y=predprobMandT,color="Married and Trained"),size=1)+
  labs(x = "Age_c", y = "Predicted values of Wage1978 - Wage1974") +
  theme_classic()+
...
par(mfcol=c(2,1))
binnedplot(y=wages$nonZero78[wages$treat==0],x=wages$age[wages$treat==0],xlab="age",ylab="nonZero",ylim=c(0,1),main = "Binned age vs Nonzero78 Not trained")
...
par(mfcol=c(2,1))
binnedplot(y=wages$nonZero78[wages$treat==0],x=wages$educ[wages$treat==0],xlab="education years",ylab="nonZero",ylim=c(0,1),main = "Binned education year vs Nonzero78 Not trained")
...
par(mfcol=c(2,1))
binnedplot(y=wages$nonZero78[wages$treat==0],x=wages$re74[wages$treat==0],xlab="education years",ylab="nonZero",ylim=c(0,1),main = "Binned re74 vs Nonzero78 Not trained")
...
## prediction curve
wages_newdata <- data.frame(age_c=round(seq(min(wages$age_c),max(wages$age_c),0.5),1),re74_c=0)
wages_newdata$age_c2 <- wages_newdata$age_c^2
wages_newdata$age_c3 <- wages_newdata$age_c^3
# not black, not trained
wages_newdata$black_fac<- factor("Not Black",levels=levels(wages$black_fac))
wages_newdata$train_fac <- factor("Not trained",levels= levels(wages$train_fac))
predprobbaseline <- predict(glmmodel1,wages_newdata,type='response')

# black, not trained
wages_newdata$black_fac<- factor("Black",levels=levels(wages$black_fac))
wages_newdata$train_fac <- factor("Not trained",levels= levels(wages$train_fac))
predprobBlack <- predict(glmmodel1,wages_newdata,type='response')

# not black, trained
wages_newdata$black_fac<- factor("Not Black",levels=levels(wages$black_fac))
wages_newdata$train_fac <- factor("Trained",levels= levels(wages$train_fac))
predprobtrained <- predict(glmmodel1,wages_newdata,type='response')

# black trained
wages_newdata$black_fac<- factor("Black",levels=levels(wages$black_fac))
wages_newdata$train_fac <- factor("Trained",levels= levels(wages$train_fac))
predprobBlackTrained <- predict(glmmodel1,wages_newdata,type='response')


### Plot lines by site
colors <- c("Not Black and Not trained" = "red", "Black and Not trained" = "blue", "Not Black and Trained" = "purple","Black and Trained"="orange")

p2<-ggplot(wages_newdata, aes(x = age_c)) +
  geom_line(aes(y = predprobbaseline, color = "Not Black and Not trained"),size = 1) +
  geom_line(aes(y=predprobBlack,color="Black and Not trained"),size=1)+
  geom_line(aes(y=predprobtrained,color="Not Black and Trained"),size=1)+
  geom_line(aes(y=predprobBlackTrained,color="Black and Trained"),size=1)+
  labs(x = "Age_c", y = "Predicted Expected Odds of Positive Wage in 1978") +
  theme_classic()+
  theme(legend.position="right")
p2
