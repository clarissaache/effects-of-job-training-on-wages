...
wages$treat_fac <-factor(wages$treat,levels=c(0,1),labels=c("Not trained","Trained"))
...
wages$educ1[wages$educ<6]<-"Low"
wages$educ1[wages$educ>=6 & wages$educ<9]<-"Middle"
wages$educ1[wages$educ>=9 & wages$educ<=13]<-"High school"
wages$educ1[wages$educ>=13 & wages$educ<17]<-"College"
wages$educ1[wages$educ>=17]<-"Higher educ"
...
Looking at the following tables, for people who got treated who have some small numbers in
-hispanics who got treated(11)
-people who were below poverty level and got treated (45)
```{r}

table(wages[,c("treat_fac","married_fac")])
table(wages[,c("treat_fac","black_fac")])
table(wages[,c("treat_fac","hispan_fac")])
table(wages[,c("treat_fac","nodegree_fac")])
table(wages[,c("treat_fac","nonZero78_fac")])
```
...
Talk about how we have 2 points outlier that are higher that 30.000. once removed the distribution is way linear.
```{r}
...
```
```{r}
#Univariate analysis for each predictor
...
ggplot(wages, aes(treat_fac))+geom_bar()
```
...
#+geom_hline(aes(yintercept=median(wages[wages$nodegree==0,]$differ7874)), colour="#990000")+geom_hline(aes(yintercept=median(wages[wages$nodegree==1,]$differ7874)), colour="darkblue")
  #scale_y_continuous(breaks = round(seq(min(wages$differ7874), max(wages$differ7874), by = 3000),1))

```
```{r} 
# interaction between categorical variables
ggplot(wages, aes(x=treat_fac,y=differ7874,fill=differ7874))+
  geom_boxplot()+
  scale_fill_brewer(palette = "Reds")+
...
ggplot(wages,aes(x=age,y=differ7874))+ geom_point()+geom_smooth(method="lm",col="red3")+labs(title="Age vs Diff wages 78-74 by treatment",y="wage78-74",x="age") +facet_wrap(~treat_fac)
...
ggplot(wages,aes(x=educ,y=differ7874))+ geom_point()+geom_smooth(method="lm",col="red3")+labs(title="Education vs Diff wages 78-74 by training",y="wage78-74",x="education year") +facet_wrap(~treat_fac)
...
fullModel <- lm(differ7874~age_c*educ1*married_fac*treat_fac*black_fac*hispan_fac*nodegree_fac,data=wages)
...
model1 <- lm(formula = differ7874 ~ treat_fac + age_c + married_fac + treat_fac:age_c + 
    age_c:married_fac, data = wages[-132,])
summary(model1)
confint(model1)

plot(model1)

ggplot(wages[-132,],aes(x=age_c, y=model1$residual)) +
geom_point(alpha = .7) + geom_hline(yintercept=0,col="red3") + theme_classic() +
labs(title="Residuals vs age_c",x="age_c",y="Residuals")

ggplot(wages[-132,],aes(x=educ, y=model1$residual)) +
geom_point(alpha = .7) + geom_hline(yintercept=0,col="red3") + theme_classic() +
labs(title="Residuals vs age_c",x="age_c",y="Residuals")
...
# add interaction terms, and step wise selection
nullModel <- lm(differ7874~treat_fac,data= wages)
fullModel <- lm(differ7874~age_c*educ_fac*married_fac*treat_fac*black_fac*hispan_fac*nodegree_fac,data=wages)
stepwise <- step(nullModel,scope = formula(fullModel),direction="both",trace=0)#,k=log(n))
stepwise$call
model1 <- lm(formula = differ7874 ~ treat_fac + age_c + married_fac + treat_fac:age_c + 
...
The intercept: is the log odds of switching from zero to non-zero when treated is baseline and race is not black and the rest is ....
for black_fac blac: 
the odds ratio for black people who are trained compared to oher races who are trained, the odds ratio is 
